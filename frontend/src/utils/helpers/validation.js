...
export const storyPanelValidationSchema = Yup.object().shape({
  previewTitle: Yup.string().required('Поле обязательно'),
  previewTitleColor: Yup.string().required('Поле обязательно'),
  previewGradient: Yup.string().required('Поле обязательно'),
  previewUrl: Yup.mixed()
    .test('fileFormat', 'Неподходящий тип изображения', checkFileFormat)
    .test('fileSize', `Максимальный допустимый размер - ${MAX_IMAGE_SIZE}КБ`, checkFileSize)
    .test('fileSides', `Максимальный допустимый размер - ${MAX_IMAGE_WIDTH}x${MAX_IMAGE_HEIGHT}px`, checkFileSides),
  storyFrames: Yup.array().of(
    Yup.object().shape({
      title: Yup.string()
        .required('Поле обязательно')
        .test('titleStrings', `Максимальное кол-во строк в заголовке - ${framesRules.length}`, checkTitleStrings)
        .test('titleLength', checkTitleLength)
        .test('titleStringsLength', checkTitleStringsLength),
      text: Yup.string()
        .notRequired()
        .when('title', {
          is: titleValue => titleValue && checkTitleStrings(titleValue),
          then: Yup.string().test('textValidation', (value, { createError }) => {
            const titleStringsCount = getStrings(value).length;
            const rule = framesRules[titleStringsCount - 1];
            ...
            if (!checkStringsCount(value, rule.maxTextStrings)) {
              return createError({ message: `Максимальное кол-во строк в тексте - ${rule.maxTextStrings}` });
            }

            if (!checkLength(value, rule.maxTextLength)) {
              return createError({ message: `Максимальное кол-во символов в тексте - ${rule.maxTextLength}` });
            }

            const strings = getStrings(value);
            for (const string of strings) {
              if (string.length < rule.minLength) {
                return createError({ message: 'Пустая строка' });
              }
            }
